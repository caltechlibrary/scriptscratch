...
for directory in *; do
    ...
    if [ -f "$directory/${directory}_prsv.mov" ]; then
        if ! ffmpeg -hide_banner -nostats -i "$directory/${directory}_prsv.mov" -map 0 -dn -c:v ffv1 -level 3 -g 1 -slicecrc 1 -slices 4 -c:a copy "$2/$directory/${directory}_prsv.mkv" 2>&1 | tee "$2/$directory/${directory}_prsv.ffmpeg.log"; then
            echo -e "\n❌ FFMPEG TRANSCODE FAILED"
            exit 1
        else
            # create raw audio & video stream MD5 checksum of *_prsv.mkv
            if [ -f "$2/$directory/${directory}_prsv.mkv" ]; then
                ffmpeg -i "$2/$directory/${directory}_prsv.mkv" -f hash -hash md5 "$2/$directory/${directory}_prsv.raw.md5"
            fi
        fi
    fi
    ...
    raw_mkv_file_md5=$(cat "$2/$directory/${directory}_prsv.raw.md5")
    ...
    if ! [ "$raw_mov_calculated_md5" == "$raw_mkv_file_md5" ]; then
        ...
        echo -e "$2/$directory/${directory}_prsv.raw.md5:\t$raw_mkv_file_md5"
        exit 1
    else
        echo -e "\n✅ RAW MD5 MATCH"
        echo -e "$directory/${directory}_prsv.mov:\t$raw_mov_calculated_md5"
